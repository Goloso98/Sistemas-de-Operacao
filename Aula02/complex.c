...
complex* complex_div(complex* z, complex* w) {
   complex* numerator = complex_mul(z, complex_conj(w));
   double denominator = complex_re(complex_mul(w, complex_conj(w)));

   return complex_new(numerator->x/denominator, numerator->y/denominator);
}
